...
const MobileNavbar = () => {
  ...
  return (
    <motion.div 
       initial = {{y:150, x: "-50%", }}
       animate={!isloading && {y:0,x: "-50%",}}
       transition={{delay:0}}
    className='NaviBar'>
        <Link to={"/ValidatorDashboard"} className="feed" style={{display: searchBar ? 'flex' : 'none',textDecoration:"none", color:"white"}}>
           <img style={{width:"28px",height:"28px"}} src={validatorImage}></img>
           <p style={{}}>Validator</p>
        </Link>
        <Link to={"/"}  className="dispute" style={{display: searchBar ? 'flex' : 'none',textDecoration:"none", color:"white"}}>
        ...
        <Link to={`Profile/${localStorage.getItem('username')}`} className="profile" style={{display: searchBar ? 'flex' : 'none',textDecoration:"none", color:"white"}}>
