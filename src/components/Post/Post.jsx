...
export default function Post({name,address,createdTime,isfollowed,postHead,postBody,tags,type}){
    ...
    return(
        <div className="Post">
        {interactionMenu&& (type == "Challenge"? 
        <>
           {showDisclaimer && !disclaimer && <Disclaimer closeMenu={setShowDisclaimer} />}
           {(!showDisclaimer || disclaimer) &&  <InteractionB closeMenu={setInteractionMenu} />}
        </>:<InteractionA closeMenu={setInteractionMenu}/> )}

        {interactionMenu && (<div onClick={()=>{setInteractionMenu(false)}} className="BackdropEffect"> </div>)}
            ...
            <div className="PostBody">
                 ...
                 <button
                  onClick={()=>setInteractionMenu(true)}
                 >VIEW 
